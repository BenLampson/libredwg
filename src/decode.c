...
static int decode_R2004_header (Bit_Chain *restrict file_dat,
                                Dwg_Data *restrict dwg);
static int read_R2004_section_map (Bit_Chain *restrict dat,
                                   Dwg_Data *restrict dwg);
static int read_R2004_section_info (Bit_Chain *restrict dat,
                                    Dwg_Data *restrict dwg,
                                    uint32_t comp_data_size,
                                    uint32_t decomp_data_size);
static int read_2004_section_header (Bit_Chain *restrict dat,
                                     Dwg_Data *restrict dwg);
static int read_2004_section_classes (Bit_Chain *restrict dat,
                                      Dwg_Data *restrict dwg);
static int read_2004_compressed_section (Bit_Chain *dat,
                                         Dwg_Data *restrict dwg,
                                         Bit_Chain *sec_dat,
                                         Dwg_Section_Type type);
static int read_r2004_meta_data_stream (
                   ...
                   int *restrict stream_supported, void *restrict user)
{
  ...
  if (dwg.header.from_version >= R_2004a
  ...
  if (dwg.header.from_version == R_1_4)
    {
      ...
      error = read_pre_r2_meta_data_stream (dat, &dwg, callbacks,
                                            input_mode, user);
      dwg_free (&dwg);
      return error;
    }
  ...
  if (dwg.header.from_version == R_2_10)
    {
      ...
      error = read_pre_r10_meta_data_stream (dat, &dwg, callbacks,
                                             input_mode, user);
      dwg_free (&dwg);
      return error;
    }
    ...
    const Dwg_Stream_Callbacks_Ex *restrict callbacks, void *restrict user)
{
  ...
  if (!dwg || !object_dat || !info || !callbacks
...
stream_object_record_size (const Dwg_Stream_Object_Info *restrict info,
                           ...
                           size_t *restrict object_size)
{
  int error = stream_object_span_size (info, object_start, total_size,
                                       object_size);
  if (error)
    return error;
                                   ...
                                   Dwg_Stream_Object_Info *restrict info)
{
  ...
  if (info->type >= 500
    ...
    {
      info->supertype
          = stream_fixed_type_is_entity (fixedtype) ? DWG_SUPERTYPE_ENTITY
                                                    : DWG_SUPERTYPE_OBJECT;
    }
  ...
  do
    {
      ...
      section_size = bit_read_RS_BE (dat);
      if (section_size > 2040
          || (size_t)section_size + 2 > lastmap - startpos)
        {
          error = DWG_ERR_VALUEOUTOFBOUNDS;
          goto done;
        }
          ...
          if (callbacks->decoded_object)
            {
              ...
              size_t object_size;
              object_error = stream_object_record_size (&info, last_offset,
                                                        dat->size,
                                                        &object_size);
              if (object_error)
                {
...
{
  const R2004_Stream_Handle_Entry *left
      = (const R2004_Stream_Handle_Entry *)a;
  ...
  for (unsigned int i = 0; i < lit_length; ++i)
    {
      ...
      if (src->byte >= src->size)
        return DWG_ERR_VALUEOUTOFBOUNDS;
      error = r2004_stream_put_byte (stream, bit_read_RC (src), target,
                                     buffer, buffer_size);
      if (error)
        return error;
    }
  ...
  if (end <= stream->output_pos)
    {
      for (size_t i = 0; i < buffer_size; i++)
        {
          int error = r2004_stream_get_history (stream, target + i,
                                                &buffer[i]);
          if (error)
            return error;
        }
  ...
  if (target < stream->output_pos)
    {
      ...
      for (size_t pos = target; pos < available_end; pos++)
        {
          int error;
          error = r2004_stream_get_history (stream, pos,
                                            &buffer[pos - target]);
          if (error)
            return error;
        }
  ...
  while (stream->output_pos < end)
    {
      ...
      while (stream->output_pos < es.fields.address)
        {
          error = r2004_stream_put_byte (stream, 0, target, buffer,
                                         buffer_size);
          if (error)
            return error;
        }
          ...
          for (uint32_t i = 0; i < size; i++)
            {
              error = r2004_stream_put_byte (
                  stream, stream->dat->chain[address + 32 + i], target,
                  buffer, buffer_size);
              if (error)
                return error;
            }
      ...
      while ((long)(hdl_dat.byte - startpos) < (long)section_size)
        {
          ...
          if (!handleoff || handleoff > max_handles - last_handle
              || (offset > -4 && offset < prevsize))
            LOG_WARN ("Ignore invalid handleoff (@%" PRIuSIZE ")", oldpos);
          object_error = stream_add_handle_value (
    ...
    Dwg_Stream_Input_Mode input_mode, void *restrict user)
{
  ...
  if (error < DWG_ERR_CRITICAL)
    error |= read_2004_section_classes (dat, dwg);
  if (error < DWG_ERR_CRITICAL)
    error |= read_2004_section_handles_stream (dat, dwg, callbacks,
                                               input_mode, user);
  return error;
}
  ...
  if (opts_r11 & OPTS_R11_POLYLINE_HAS_FLAG)
    {
      ...
      if (pline_flag & FLAG_POLYLINE_3D)
        return dwg_decode_POLYLINE_3D (dat, obj);
      if (pline_flag & FLAG_POLYLINE_MESH)
        return dwg_decode_POLYLINE_MESH (dat, obj);
      if (pline_flag & FLAG_POLYLINE_PFACE_MESH)
        return dwg_decode_POLYLINE_PFACE (dat, obj);
      return dwg_decode_POLYLINE_2D (dat, obj);
    }
  ...
  if (opts_r11 & OPTS_R11_VERTEX_HAS_FLAG)
    {
      ...
      if (vertex_flag & FLAG_VERTEX_MESH
        ...
        return dwg_decode_VERTEX_PFACE (dat, obj);
      if (vertex_flag & FLAG_VERTEX_MESH)
        return dwg_decode_VERTEX_MESH (dat, obj);
      if (vertex_flag & FLAG_VERTEX_PFACE_MESH)
        return dwg_decode_VERTEX_PFACE_FACE (dat, obj);
      if (vertex_flag & FLAG_VERTEX_3D)
        return dwg_decode_VERTEX_3D (dat, obj);
      return dwg_decode_VERTEX_2D (dat, obj);
    }
    ...
    const Dwg_Section_Type_r11 section, BITCODE_BL *restrict index)
{
  ...
  dat->byte = dwg->header.from_version >= R_11 ? tbl->address - 16
                                                : tbl->address;
  dat->bit = 0;
  ...
  if (blocks_size)
    {
      ...
      if (error >= DWG_ERR_CRITICAL)
        return error;
      error = emit_pre_r13_object_range_stream (
          dwg, callbacks, input_mode, user, start_index, &index);
      if (error)
        return error;
    }
