...
dwg_decode_add_object_ref (Dwg_Data *restrict dwg, Dwg_Object_Ref *ref)
{
  ...
  const BITCODE_BL refs_per_realloc
      = dwg->opts & DWG_OPTS_STREAM_DECODE ? STREAM_REFS_PER_REALLOC
                                           : REFS_PER_REALLOC;
...
/* Keep the old pre-R13 decoders private while allowing the extracted Stream
   reader to use the same implementations. */
#define DWG_STREAM_PRE_R13_DECODER(token)                                    \
  int                                                                        \
  dwg_stream_decode_pre_r13_##token (Bit_Chain *restrict dat,                \
                                     Dwg_Object *restrict obj)               \
  {                                                                          \
    return dwg_decode_##token (dat, obj);                                    \
  }
