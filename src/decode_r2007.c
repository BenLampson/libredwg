...
static int
read_data_section_page (Bit_Chain *page_dat, Bit_Chain *dat,
                        r2007_page *restrict pages_map,
                        r2007_section_page *restrict section_page)
{
  r2007_page *page;
  BITCODE_RC *decomp, *decomp_end;
  int error = 0;
  ...
  if (section_page->uncomp_size >= DBG_MAX_SIZE
      || section_page->comp_size >= DBG_MAX_SIZE)
    {
  ...
  if (section_page->comp_size != section_page->uncomp_size)
    ...
    {
      if (section_page->uncomp_size > dat->size - dat->byte)
        {
          free (decomp);
          LOG_ERROR ("Invalid section page size %ld",
                     (long)section_page->uncomp_size);
          return DWG_ERR_VALUEOUTOFBOUNDS;
        }
      memcpy (decomp, &dat->chain[dat->byte], section_page->uncomp_size);
    }

  page_dat->chain = decomp;
  page_dat->size = (size_t)section_page->uncomp_size;
  page_dat->version = dat->version;
  page_dat->from_version = dat->from_version;
  page_dat->opts = dat->opts;
  page_dat->codepage = dat->codepage;
  return 0;
}
