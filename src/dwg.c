...
/** dwg_stream_file
 * Backward-compatible object-info streaming API.
 */
EXPORT int
                 ...
                 void *restrict user)
{
  ...
  callbacks_ex.object = callbacks->object;
  callbacks_ex.flags = callbacks->flags;
  return dwg_stream_file_ex (filename, &callbacks_ex, user);
}
